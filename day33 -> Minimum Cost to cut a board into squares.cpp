...
Your Task:  
You don't need to read input or print anything. Your task is to complete the function minimumCostOfBreaking()
which takes a string S and returns an integer as output.
...
public:
    int minimumCostOfBreaking(vector<int> X, vector<int> Y, int M, int N){
        ...
        int totalCost = 0,vCost = 1,hCost = 1;
        ...
        while(!pq.empty()){
            if(pq.top().second == 'x'){
                totalCost += pq.top().first * vCost;
                hCost++;
            }
            else{
                totalCost += pq.top().first * hCost;
                vCost++;
            }
            pq.pop();
        }
