...
You start at the upper-left corner of the grid (1, 1) and you have to reach the bottom-right corner (n, m) such that you can only move in the right 
or down direction from every cell.
...
Your Task:  
You don't need to read input or print anything. Your task is to complete the function uniquePaths() which takes 2 integers n, and m, 
and a matrix of size n*m as input and returns the number of unique paths from cell (1,1) to (n,m) modulo (109+7)
  ...
  public:
    ...
    int sol(int i,int j,int n,int m,vector<vector<int>> &grid,vector<vector<int>> &dp){
       ...
       if(i>=n || j>=m || grid[i][j]==0) return 0;
        ...
        int down=sol(i+1,j,n,m,grid,dp);
        
        int right=sol(i,j+1,n,m,grid,dp);
