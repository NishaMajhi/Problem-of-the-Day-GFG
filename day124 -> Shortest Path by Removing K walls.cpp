...
Your Task:
The task is to complete the function shotestPath() which takes three integers n, m, and k and also a matrix of size n*m as input
and returns the minimum number of steps to walk from the top left corner to the bottom right corner.
...
class Solution {
  public:
    int shotestPath(vector<vector<int>> arr, int n, int m, int k) {
        ...
        // This vector stores the number of obstacles that we can still
        // remove after walking through that cell
        ...
        while(q.empty() == false)
        {
            ...
            if(arr[x][y] == 1)
             ...
             // If this cell is already visited with a K value lesser than this one, 
             //  we would want to save Ks for future use, so we continue
