...
Your Task:
You don't need to read input or print anything. Your task is to complete the function convert() which takes the array of integers arr and n as parameters
and make changes in the given array.
...
public:	
	...
	void convert(int arr[], int n) {
	   ...
	   //take adummy vector to store all the array elements
	   vector<int> temp(n);
	   for(int i=0;i<n;i++){
	       temp[i]=arr[i];
	   }
	   
	   //sort the element of dummy vector
	   sort(temp.begin(),temp.end());
	   
	   //map element of sorted dummy vector
	   unordered_map<int,int> mp;
	   for(int i=0;i<n;i++){
	       mp[temp[i]]=i;
	   }
	   
	   //find the sorted position of actual array element in map
	   for(int i=0;i<n;i++){
	       arr[i] = mp[arr[i]];
	   }
