...
In each minimising operation, geek can remove any two consecutive characters if they are of the form {"12", "21", "34", "43", "56", "65", "78", "87", "09", "90"}.
...
Your Task:  
You don't need to read input or print anything. Complete the functionminLength() 
which takes N and S as input parameters and returns the the minimum possible length of the string.
    ...
    int minLength(string s, int n) {
        ...
        for(int i=0;i<n;i++){
            if(!st.empty()){
                ...
                temp += st.top();
                temp += s[i];
                if(temp == "21" || temp == "12" || temp == "34" || temp=="43" || temp =="56" || temp =="65" ||
                    temp =="78" || temp =="87" || temp == "09" || temp =="90"){
                        st.pop();
                        continue;
                    }
            }
