...
class Solution {
  public:
    ...
    int sum(Node* root){
        ...
        if(root==NULL)
            return 0;
        //find left hand side value 
        int leftValue = sum(root->left);
        
        //find right hand side value
        int rightValue = sum(root->right);
        
        //get current root data for calculating parent node value
        int currRootValue = root->data;
        
        //calculate current root value
        root->data = leftValue + rightValue;
        
        //return new cureent node value + previous value of current node 
        return leftValue+rightValue+currRootValue;
