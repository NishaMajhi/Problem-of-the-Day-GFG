...
Your Task:  
You don't need to read input or print anything. Your task is to complete the function lexicographically Largest() which takes the array arr[],
and its size N as input parameters and returns the array b.
...
class Solution{
    public:
        vector <int> lexicographicallyLargest(vector <int> &a,int n){
            ...
            while(++en < n){
                if( (a[en] & 1) != (a[en-1] & 1 ) ){
                    sort(a.begin()+st, a.begin()+en,greater<int>() );
                    st=en;
                }
            }
